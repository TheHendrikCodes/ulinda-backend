...
public class DemoDataService {
    ...
    private void loadEmployeeData() {
        GetModelsResponse modelsResponse = modelService.getModels();
        ...
        for (ModelDto model : modelsResponse.getModels()) {
            if (model.getName().equals("Employees")) {
                ...
                // Create 10,000 employee records
                for (int i = 0; i < 1000; i++) {
                    ...
                    modelService.createRecord(modelId, fieldValues);
                }
                break;
            }
        }
    ...
    private void loadMotorbikeData() {
        ...
        for (ModelDto model : modelsResponse.getModels()) {
            if (model.getName().equals("Motorbikes")) {
                ...
                // Create 1,000 motorbike records
                for (int i = 0; i < 1000; i++) {
                    Map<UUID, Object> fieldValues = new HashMap<>();
    ...
    private void linkEmployeesToPayslips() {
        ...
        GetModelResponse employeesModelResponse = modelService.getModel(employeesModelId);
        ...
        for (int i = 0; i < Math.min(employees.size(), payslips.size()); i++) {
            ...
            modelService.linkRecords(linkRecordsRequest);
        }
    }
    ...
    private void linkEmployeesToMotorbikes() {
        ...
        for (RecordDto employee : employees) {
            ...
            // Link to the first 50 motorbikes after shuffling
            int bikesToLink = Math.min(50, availableMotorbikes.size());
